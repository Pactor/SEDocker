...
RUN useradd -m -s /bin/bash wine && \
    ...
    mkdir -p /home/wine/scripts /home/wine/torch && \
    chown -R wine:wine /home/wine

# Copy your helper scripts
WORKDIR /home/wine/scripts
COPY setup_server.sh setup_wine.sh torch_run.sh startup.sh select_scenario.sh configure_game.sh select_plugins.sh setup_mods.sh install_world.sh init_docker.sh setup_ports.sh ./
